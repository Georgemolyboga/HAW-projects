...
public class Background {
    ...
    private void generateDecorations() {
        ...
        generateBirds(RandomNumber.between(MIN_BIRDS, MAX_BIRDS));
    }
    ...
    private void generateBirds(int count) {
        birds.clear();
        for (int i = 0; i < count; i++) {
            int x = RandomNumber.between(0, width);
            int y = RandomNumber.between(50, height/2);
            int size = RandomNumber.between(10, 20);
            birds.add(new Bird(x, y, size));
        }
    }
    ...
    private class Bird {
        ...
        void draw(Graphics g) {
            ...
            // Body
            g.drawArc(x, y, size, size/2, 0, 180);
            // Wings
            g.drawArc(x + size/2, y - size/4, size, size/2, 45, 180);
            g.drawArc(x + size/2, y + size/4, size, size/2, -45, -180);
        }
    }
